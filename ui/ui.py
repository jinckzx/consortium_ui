    ...
    if st.button("Run Consortium"):
        ...
        with st.spinner("Running consortium process..."):
            ...
            try:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    confidence_value = data['synthesis']['confidence']
                    confidence_color = "#2ecc71" if confidence_value >= 0.9 else "#e74c3c"
                    st.markdown(f"<h3 style='color:{confidence_color};'>Final Confidence: {confidence_value:.2%}</h3>", 
                                unsafe_allow_html=True)
                
                with col2:
                    st.metric("Iterations", data['metadata']['iteration_count'])
                
                # Add the confidence plot
                st.plotly_chart(plot_confidence_scores(), use_container_width=True)
